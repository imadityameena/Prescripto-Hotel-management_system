...
const MyProfile = () => {
    ...
    // (27.1) Implement editing functionality based on condition ✅
    // State to toggle between view and edit modes for the profile form.
    ...
    const updateUserProfileData = async () => {
        try {
            // (44.2) Implementing multiple middleware functions for user authentication and data update ✅
            // FormData is used to handle both text data and the optional image file upload.
            ...
            // (44.3) Updating user profile with optional image ✅
            image && formData.append('image', image)

            const { data } = await axios.post(backendUrl + '/api/user/update-profile', formData, { headers: { token } })

            // (44.4) Creating logic to update user profile data in the database ✅
            if (data.success) {
                toast.success(data.message)
                await loadUserProfileData() // Re-fetch data to sync with the database
                setIsEdit(false)
                setImage(false)
            } else {
    ...
    return userData ? (
        <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
            ...
            {/* // (28.1) Adding UI design for profile page and functionality for editing profile picture ✅ */}
            {/* // (44.5) Adding image tags and input field with type file to select images in HTML form ✅ */}
            {/* Conditionally renders either a static image or a file input for changing the profile picture. */}
            {isEdit
                ? <label htmlFor='image' >
            ...
            {/* // (27.2) Implement editing functionality based on condition ✅ */}
            {/* Conditionally renders a text input or static text for the user's name. */}
            {isEdit
            ...
            {/* // (27.3) Updating address and basic information on the web page ✅ */}
            <div>
            ...
            <div>
                {/* // (27.4) Adding gender selection and date input using MERN Stack ✅ */}
            ...
            {/* // (27.5) Implement editing functionality based on condition ✅ */}
            {/* The button toggles between "Edit" and "Save" based on the `isEdit` state. */}
            <div className='mt-10'>
                {isEdit
